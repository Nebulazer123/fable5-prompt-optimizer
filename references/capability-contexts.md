...
- Preparedness: if the skill/repo already has pending edits, validation output, examples, or open decisions, ask Codex to finish/capture cheap nearby state and create a context packet before Fable reviews it.
...
- Preparedness: if Codex is near a checkpoint, finish or capture the small implied step first: save the current change, capture `git status`/diff, run the relevant quick check, or note why it cannot be run.
...
- Preparedness: include screenshots, current viewport notes, visual QA results, relevant assets, and known design feedback in a context packet when Fable needs to critique or direct the next pass.
...
- Preparedness: collect exact file paths, page/image references, extracted tables, review notes, and uncertainty areas into the packet when the source set is larger than a single attachment.
...
- Preparedness: give the orchestrator a prepared packet with lanes, known evidence, current state, and desired synthesis so subagents start from the same snapshot.
